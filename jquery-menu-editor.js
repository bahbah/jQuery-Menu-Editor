...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        ...
        showIcons: true,
        iconPicker: { cols: 5, rows: 5, footer: true, iconset: "fontawesome5" },
        maxLevel: -1,
        listOptions: { 
            ...
            onChange: function(cEl) {
                cEl.toggleClass('pe-0',         cEl.parent().get(0) != $main.get(0));
                cEl.toggleClass('pe-2 pe-sm-3', cEl.parent().get(0) == $main.get(0));
    ...
    $main.on('click', '.btnOut', function (e) {
        ...
        $li.toggleClass('pe-0',         $ul.get(0) != $main.get(0));
        $li.toggleClass('pe-2 pe-sm-3', $ul.get(0) == $main.get(0));
        MenuEditor.updateButtons($main);
        $main.updateLevels();
    });
    $main.on('click', '.btnIn', function (e) {
        ...
        if ($prev.length > 0) {
            ...
            if ($ul.length > 0)
                ...
                var $ul = $('<ul>').addClass('ps-0').css('padding-top', '10px');
                $prev.append($ul);
                $ul.append($li);
    ...
    function createMenu(arrayItem, depth) {
        var level = (typeof (depth) === 'undefined') ? 0 : depth;
        var $elem = (level === 0) ? $main : $('<ul>').addClass('ps-0').css('padding-top', '10px').data("level", level);
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>').addClass('list-group-item ps-2 ps-sm-3');
            // Remove right padding from items not on top level,
            // or else their buttons won't align with their parent's
            $li.toggleClass('pe-0',         $elem.get(0) != $main.get(0));
            $li.toggleClass('pe-2 pe-sm-3', $elem.get(0) == $main.get(0));
            $li.data(itemObject);
    ...
    this.add = function($parentItem, index){
        ...
        if ($ul.has($parentItem).length) {
            ...
            if (!$ul.length) {
                ...
                $ul = $('<ul>').addClass('ps-0').css('padding-top', '10px').data('level', parentLevel + 1);
